...
Client会将请求提交给本地的Raft节点。但是就像上面提到的，它们仅仅在Leader上处理，所以Followers会将请求转发给Leader（论文6.2节）。为了避免
...
关于读请求，仅在Leader上处理，不需要Raft的日志的复制。但是为了确保强一致性，Leader会在处理读请求的时候，会通过一次心跳来避免脏读（避免其他地方选举出来了新的Leader而导致的Leader身份失效）（论文8节）。一旦可以确认Leader的身份，Leader就会将读取结果返回给Client。
