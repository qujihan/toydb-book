...
#reference-block("Rust中的写时复制")[
  ...
  fn main() {
      let borrowed_data = "This is a test string without the keyword.";
      let owned_data = "This string contains foo and will be modified.";
  ...
  在说明这几个值之前先说一下 `cow` （copy on wirte，写时复制），这是一个非常常见的概念。`Cow`是一个枚举类，有两个值，`Borrowed`和`Owned`。`Borrowed`表示借用，`Owned`表示拥有。当一个`Cow`是`Borrowed`的时候，它是一个引用，当一个`Cow`是`Owned`的时候，它是一个拥有者。通过`Cow`可以实现写时复制，也就是说，当一个`Cow`是`Borrowed`的时候，如果需要修改，就会复制一份，然后修改这份拷贝，这样就不会影响原来的数据。
